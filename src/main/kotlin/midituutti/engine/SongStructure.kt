 ...
 */
data class Measure(
    val number: Int, val start: Tick, val timeSignature: TimeSignature,
...
class SongStructure(val measures: List<Measure>) {
    ...
    private class Parser(val ticksPerBeat: Int) {
        ...
        private tailrec fun parseRec(
            ...
            rem: List<MidiMessage>
        ): List<Measure> {
            if (rem.isEmpty()) {
                return acc + Measure(
                    acc.size + 1, measureStart, currTimeSignature, currTempo,
                ...
                return if (withinMeasure(message.ticks(), currTimeSignature, measureStart)) {
                    ...
                    parseRec(
                        acc, nextTimeSignature, nextTempo, nextPrevAdjustmentsReversed, measureStart,
                    ...
                    val intermediateMeasures = generateSequence(firstIntermediateMeasure, { it + 1 })
                        ...
                        .map { mt ->
                            Triple(
                                mt.first,
                                mt.second,
                                if (mt.first == firstIntermediateMeasure) measureEvents else emptyList()
                            )
                        }
                        .map { mte ->
                            Measure(
                                mte.first, mte.second, currTimeSignature, currTempo,
        ...
        private fun addPossibleAdjustmentEvent(
            prevAdjustmentsReversed: Map<Int, Map<String, List<MessageEvent>>>,
            event: MessageEvent
        ): Map<Int, Map<String, List<MessageEvent>>> {
            return when (event.message) {
                is ChannelAdjustmentMessage -> {
                    ...
                    prevAdjustmentsReversed.plus(
                        Pair(
                            channel,
                            channelAdjustments.plus(
                                Pair(
                                    typeId,
        ...
        private fun collectLatestTypeSpecificAdjustments(
            prevAdjustmentsReversed: Map<Int, Map<String, List<MessageEvent>>>,
            atOrBefore: Tick
        ): Map<Int, List<MessageEvent>> {
